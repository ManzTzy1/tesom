    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `*Selamat Datang Di ${metadata.subject} @${num.split("@")[0]}*\n\n*Intro Dulu Ya 🙂*\n➤ _Nama:_\n➤ _Askot:_\n➤ _Umur:_\n➤ _Hobi:_\n\n *Semoga Betah Ya Kk @${num.split("@")[0]} Dan Patuhi Deskripsi Group*` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Yah Si Beban Group @${num.split("@")[0]} Out, Semoga Hidup Mu Di Penuhi Dengan Ke Sengsaraan 🗿` })
                }
            }
        } catch (err) {
            console.log(err)
